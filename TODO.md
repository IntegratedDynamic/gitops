...
`services/platform/openbao/applications/local/chart.vendor.yaml` a encore `dev: enabled: true`.  
...
La config S3 est posée (`services/platform/openbao/applications/local/chart.vendor.yaml`), mais le CronJob ne peut pas s'authentifier à OpenBao sans :
...
Réalisé via `services/platform/openbao/init/` : Job-hook ArgoCD (`hook: Sync`,
...
- [x] `snapshotAgent` configuré dans `services/platform/openbao/applications/local/chart.vendor.yaml` — pousse vers `s3://backup-dev-id` (Scaleway fr-par) toutes les 5 min
